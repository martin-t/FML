#[allow(unused_imports)] // LATER(martin-t)
...
fn literal() {
    ...
    assert_eq!(&output, "");
    assert_eq!(state.operand_stack, expected_operand_stack);
    assert_eq!(state.instruction_pointer, expected_instruction_pointer);
    assert_eq!(state.frame_stack, expected_frame_stack);
    assert_eq!(state.heap, expected_heap);
}
...
fn get_global() {
    ...
    expected_frame_stack.globals = GlobalFrame::from(vec!["skippy".to_owned()], Pointer::from(666)).unwrap();
    let expected_instruction_pointer = InstructionPointer::from(1u32);
    let expected_heap = Heap::new();
...
fn set_global() {
    ...
    state.frame_stack.globals = GlobalFrame::from(vec!["skippy".to_owned()], Pointer::from(666)).unwrap();

    step_with(&program, &mut state, &mut output).unwrap();

    let expected_operand_stack = OperandStack::from(vec![Pointer::from(42)]);
    let mut expected_frame_stack = FrameStack::from(Frame::new());
    expected_frame_stack.globals = GlobalFrame::from(vec!["skippy".to_owned()], Pointer::from(42)).unwrap();
    let expected_instruction_pointer = InstructionPointer::from(1u32);
    let expected_heap = Heap::new();
...
fn print() {
    let code = Code::from(vec![
        OpCode::Print {
            format: ConstantPoolIndex::new(0),
            arity: Arity::new(0),
        },
        OpCode::Return,
    ]);
...
fn print_array() {
    ...
    let pointer = state
        .heap
        .allocate(&mut state.frame_stack, &mut state.operand_stack, array.clone());
...
fn print_object_with_parent() {
    ...
    let pointer = state
        .heap
        .allocate(&mut state.frame_stack, &mut state.operand_stack, object.clone());
...
fn call_function_zero() {
    ...
    // assert_eq!(state.globals, FnvHashMap::default(), "test globals");
    // assert_eq!(state.instruction_pointer, Some(Address::from_usize(0)), );
    // assert_eq!(state.frames, vec!(LocalFrame::empty(),
    //                               LocalFrame::from(Some(Address::from_usize(2)), vec!())));
...
fn get_slot() {
    ...
    let head_index = state.heap.allocate(
        &mut state.frame_stack,
        &mut state.operand_stack,
        HeapObject::from(
            Pointer::Null,
...
fn call_method_zero() {
    ...
    let heap_index = state
        .heap
        .allocate(&mut state.frame_stack, &mut state.operand_stack, receiver.clone());
...
fn call_method_three() {
    ...
    state.instruction_pointer.set(Some(Address::from_usize(1)));
    let pointer = Pointer::from(state.heap.allocate(
        &mut state.frame_stack,
        &mut state.operand_stack,
        receiver.clone(),
    ));
    ...
    //
    // assert_eq!(&output, "");
    // assert_eq!(state.operands, Vec::new());
    // assert_eq!(state.globals, FnvHashMap::default(), "test globals");
    // assert_eq!(state.instruction_pointer, Some(Address::from_usize(0)), );
    // assert_eq!(state.frames, vec!(LocalFrame::empty(),
...
//
//     // assert_eq!(&output, "");
//     // assert_eq!(state.operands, vec!(result_pointer));
//     // assert_eq!(state.globals, FnvHashMap::default(), "test globals");
...
fn call_method_array_get() {
    ...
    let array = HeapObject::from_pointers(vec![Pointer::from(42), Pointer::from(666), Pointer::from(0)]);
    let array_index = state
        .heap
        .allocate(&mut state.frame_stack, &mut state.operand_stack, array.clone());
...
fn call_method_array_set() {
    ...
    let array = HeapObject::from_pointers(vec![Pointer::from(0), Pointer::from(1), Pointer::from(2)]);

    state.instruction_pointer.set(Some(Address::from_usize(0)));

    let pointer = Pointer::from(
        state
            .heap
